...
#define ONE_WIRE_BUS 3         // the one wire temperature readout bus
...
void sendNeptuneData()
{
  ...
  readPh(temp1);
   ...
   //   Serial.println("Constructing message to send via I2C");
   //   byte message[12];
  ...
  Firmata.sendString(response.c_str());
   ...
   //   Firmata.write(START_SYSEX);
   //   Firmata.write(NEPTUNE_DATA_RESPONSE);
   //   i = 0;
   //   while (i < 12)
   //   {
   //     byte sendByte = message[i++];
   //     Firmata.write(sendByte);  
   //   }
   //   Firmata.write(END_SYSEX);
}
