...
namespace Repsaj.Submerged.GatewayApp.Universal.Device
{
    public class DeviceFactory
    {
        ...
        {
            dynamic device = DeviceHelper.BuildDeviceStructure(deviceId, false);
        ...
        public static dynamic GetDevice(string deviceId, string key, Dictionary<string, string> values)
        {
            dynamic device = GetDevice(deviceId, key);

            foreach (var value in values)
            {
                JObject module = new JObject();
                module.Add(ModuleStatusConstants.NAME, value.Key);
                module.Add(ModuleStatusConstants.STATUS, value.Value);
                ((JArray)device.ModuleStatus).Add(module);
            }
