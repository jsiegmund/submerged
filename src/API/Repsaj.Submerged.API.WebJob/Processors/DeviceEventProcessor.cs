...
namespace Repsaj.Submerged.WebJob.Processors
{
    public class DeviceEventProcessor : IDeviceEventProcessor, IDisposable
    {
        ...
        readonly ISubscriptionLogic _subscriptionLogic;
        ...
        public DeviceEventProcessor(ILifetimeScope scope, IDeviceLogic deviceLogic, ISubscriptionLogic subscriptionLogic)
        {
        ...
        async Task StartProcessor(CancellationToken token)
        {
            try
            {
                ...
                _factory = new DeviceAdministrationProcessorFactory(_deviceLogic, _subscriptionLogic, _configurationProvider);
