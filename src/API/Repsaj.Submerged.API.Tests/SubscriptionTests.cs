...
namespace Repsaj.Submerged.APITests
{
    ...
    public class SubscriptionTests : TestBase
    {
        ...
        public async Task CanUpdateSubscriptionAsync()
        {
            ...
            dynamic result = await SubscriptionLogic.UpdateSubscriptionAsync(subscription);
        }
        ...
        {
            SubscriptionModel subscription = await SubscriptionLogic.GetSubscriptionAsync(SubscriptionUser);
            DeviceModel device = DeviceModel.BuildDevice(TankName, false);
        ...
        public async Task CanUpdateDeviceAsync2()
        {
            ...
            await SubscriptionLogic.UpdateDeviceAsync(firstDevice);
        }
        ...
        public async Task CanUpdateLatestTelemetryData()
        {
            ...
            await SubscriptionLogic.UpdateLatestTelemetryData(DeviceId, data);
        }

        [TestMethod]
        public async Task CanGetDeviceAsync2()
        {
            DeviceModel device = await SubscriptionLogic.GetDeviceAsync(DeviceId);
        ...
        public async Task CanAddModule()
        {
            ...
            ModuleModel cabinetModule = ModuleModel.BuildModule("Cabinet Module", "", ModuleTypes.SENSORS);

            //await SubscriptionLogic.AddModuleAsync(sensorModule, "Scubaline 240");
            await SubscriptionLogic.AddModuleAsync(cabinetModule, TankName);
        }

        [TestMethod]
        public async Task CanUpdateModule()
        {
            DeviceModel cabinetModule = await SubscriptionLogic.GetDeviceAsync(DeviceId);
            ModuleModel module = cabinetModule.Modules.First();

            await SubscriptionLogic.UpdateModuleAsync(module, cabinetModule.DeviceProperties.DeviceID);
        }

        [TestMethod]
        public async Task CanGetModulesAsync()
        {
            IEnumerable<ModuleModel> deviceModules = await SubscriptionLogic.GetModulesAsync(DeviceId);
        ...
        {
            IEnumerable<SensorModel> sensors = await SubscriptionLogic.GetSensorsAsync(DeviceId);
        ...
        {
            SensorModel sensor = SensorModel.BuildSensor("temperature1", "Temperature 1");
            await SubscriptionLogic.AddSensorAsync(sensor, DeviceId);
        }
        ...
        public async Task CanUpdateSensorAsync()
        {
            ...
            await SubscriptionLogic.UpdateSensorAsync(sensor, DeviceId);
        }

        [TestMethod]
        public async Task CanGetRelaysAsync()
        {
            var relays = await SubscriptionLogic.GetRelaysAsync(DeviceId);
        ...
        public async Task CanAddRelayAsync()
        {
            ...
            await SubscriptionLogic.AddRelayAsync(relay1, DeviceId);
            RelayModel relay2 = RelayModel.BuildModel(2, "Heating");
            await SubscriptionLogic.AddRelayAsync(relay2, DeviceId);
            RelayModel relay3 = RelayModel.BuildModel(3, "Lights");
            await SubscriptionLogic.AddRelayAsync(relay3, DeviceId);
        }

        [TestMethod]
        public async Task CanUpdateRelayAsync()
        {
            RelayModel relay = (await SubscriptionLogic.GetRelaysAsync(DeviceId)).First();
            await SubscriptionLogic.UpdateRelayAsync(relay, DeviceId);
        ...
        {
            await SubscriptionLogic.UpdateRelayStateAsync(1, true, DeviceId);
        }
    }
}
